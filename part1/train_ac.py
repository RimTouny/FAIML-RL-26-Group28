...
# Plotting helpers
def plot_reward_and_time(csv_path, save_path, title="Actor-Critic"):
    """
    Two-panel figure:
...
def run_seed(seed, device):
    ...
    print(f"\n{'─'*60}")
    print(f"  Actor-Critic | Seed {seed}")
    print(f"{'─'*60}")

    os.makedirs("models/actor_critic", exist_ok=True)
    os.makedirs("Logs/actor_critic", exist_ok=True)

    csv_path       = f"Logs/actor_critic/training_ac_seed{seed}.csv"
    eval_csv_path  = f"Logs/actor_critic/eval_ac_seed{seed}.csv"
    model_path     = f"models/actor_critic/model_ac_seed{seed}.pt"
    best_model_path = f"models/actor_critic/model_ac_seed{seed}_best.pt"
    plot_path      = f"Logs/actor_critic/learning_curve_ac_seed{seed}.png"
    ...
    policy = Policy_ac(state_dim, action_dim)
    agent = Agent_ac(policy, device=device)
    ...
    for ep in range(n_episodes):
        ...
        # Decay entropy bonus to reduce exploration over time
        if agent.policy.entropy_beta > 0.0001:
            agent.policy.entropy_beta *= 0.995
        ...
        if (ep + 1) % 100 == 0:
            print(
                ...
                f"Actor: {actor_loss:8.2f} | "
                f"Critic: {critic_loss:8.2f} | "
                f"Ent: {entropy:5.3f} | "
    ...
    plot_reward_and_time(csv_path, plot_path,title=f"Learning Curve: Actor-Critic (Seed {seed})")
...
def main():
    ...
    print(f"Training Actor-Critic on: {device}")
    print(f"Seeds: {SEEDS}")
    ...
    print("\n" + "═"*60)
    print("  Actor-Critic — Results across seeds")
    print("═"*60)

    os.makedirs("Logs/actor_critic", exist_ok=True)
    txt_path = "Logs/actor_critic/seed_results.txt"

    with open(txt_path, "w") as f:
        f.write("Algorithm: Actor-Critic\n")    
        f.write("="*50 + "\n")
        for s in SEEDS:
            m, sd, best_eval, model_path, _ = seed_stats[s]
            # Read total time from last row of CSV
            csv_path = f"Logs/actor_critic/training_ac_seed{s}.csv"
    ...
    best_model_dest = "models/actor_critic/model_ac_best.pt"
