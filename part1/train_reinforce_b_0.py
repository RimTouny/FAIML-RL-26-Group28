...
from agent_reinforce_b_0 import Agent_Reinforce, Policy_Reinforce
...
# Plotting helpers
def plot_reward_and_time(csv_path, save_path, title="REINFORCE – No Baseline"):
    """
    Two-panel figure:
...
def run_seed(seed, device):
    ...
    print(f"\n{'─'*60}")
    print(f"  REINFORCE b=0 | Seed {seed}")
    print(f"{'─'*60}")

    os.makedirs("Logs/vanilla", exist_ok=True)
    os.makedirs("models/vanilla", exist_ok=True)

    csv_path      = f"Logs/vanilla/training_vanilla_seed{seed}.csv"
    eval_csv_path = f"Logs/vanilla/eval_vanilla_seed{seed}.csv"
    model_path    = f"models/vanilla/model_vanilla_seed{seed}.pt"
    best_model_path = f"models/vanilla/model_vanilla_seed{seed}_best.pt"
    plot_path     = f"Logs/vanilla/learning_curve_baseline_0_seed{seed}.png"
    ...
    agent = Agent_Reinforce(policy, device=device)
    ...
    plot_reward_and_time(csv_path, plot_path,title=f"Learning Curve: REINFORCE – No Baseline (Seed {seed})")
...
def main():
    ...
    print(f"Training Vanilla REINFORCE on: {device}")
    print(f"Seeds: {SEEDS}")
    ...
    print("\n" + "═"*60)
    print("  REINFORCE b=0 — Results across seeds")
    print("═"*60)

    os.makedirs("Logs/vanilla", exist_ok=True)
    txt_path = "Logs/vanilla/seed_results.txt"

    with open(txt_path, "w") as f:
        f.write("Algorithm: REINFORCE – No Baseline (b=0)\n")
        f.write("="*50 + "\n")
        for s in SEEDS:
            m, sd, best_eval, model_path, _ = seed_stats[s]
            # Read total time from last row of CSV
            csv_path = f"Logs/vanilla/training_vanilla_seed{s}.csv"
    ...
    best_model_dest = "models/vanilla/model_vanilla_best.pt"
