...
from agent_reinforce_b_20 import Agent_Baseline, Policy_Baseline
...
# Plotting helpers
def plot_reward_and_time(csv_path, save_path, title="REINFORCE – Baseline b=20"):
    """
    Two-panel figure:
...
def run_seed(seed, device):
    ...
    print(f"\n{'─'*60}")
    print(f"  REINFORCE b=20 | Seed {seed}")
    print(f"{'─'*60}")

    os.makedirs("Logs/baseline", exist_ok=True)
    os.makedirs("models/baseline", exist_ok=True)

    csv_path      = f"Logs/baseline/training_baseline_seed{seed}.csv"
    eval_csv_path = f"Logs/baseline/eval_baseline_seed{seed}.csv"
    model_path    = f"models/baseline/model_baseline_seed{seed}.pt"
    best_model_path = f"models/baseline/model_baseline_seed{seed}_best.pt"
    plot_path     = f"Logs/baseline/learning_curve_baseline_20_seed{seed}.png"

    env = gym.make('Hopper-v4')
    eval_env = gym.make('Hopper-v4')   # separate env kept solely for evaluation
    ...
    policy = Policy_Baseline(obs_dim, act_dim)
    agent = Agent_Baseline(policy, device=device)
    ...
    with open(eval_csv_path, "w", newline="") as f:
        csv.writer(f).writerow([
            "training_episode", "eval_mean_return", "eval_std_return"
        ])
    ...
    plot_reward_and_time(csv_path, plot_path,title=f"Learning Curve: REINFORCE – Baseline b=20 (Seed {seed})")
...
def main():
    ...
    print(f"Training Baseline REINFORCE (b=20) on: {device}")
    print(f"Seeds: {SEEDS}")
    ...
    # Print & save results
    print("\n" + "═"*60)
    print("  REINFORCE b=20 — Results across seeds")
    print("═"*60)

    os.makedirs("Logs/baseline", exist_ok=True)
    txt_path = "Logs/baseline/seed_results.txt"

    with open(txt_path, "w") as f:
        f.write("Algorithm: REINFORCE – Baseline b=20\n") 
        f.write("="*50 + "\n")
        for s in SEEDS:
            m, sd, best_eval, model_path, _ = seed_stats[s]
            # Read total time from last row of CSV
            csv_path = f"Logs/baseline/training_baseline_seed{s}.csv" 
    ...
    best_model_dest = "models/baseline/model_baseline_best.pt"
